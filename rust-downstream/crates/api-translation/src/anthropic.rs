...
fn translate_tool_choice(body: &Value) -> Option<Value> {
    ...
    if let Some(s) = tc.as_str() {
        return match s {
            ...
            "none" => None, // Anthropic doesn't have "none" — omit tool_choice to use default
            _ => None,
        };
    }
...
mod tests {
    ...
    fn tool_choice_none_omitted() {
        ...
        let result = AnthropicTranslator.translate_request(&body).unwrap();
        assert!(
            result.body.unwrap().get("tool_choice").is_none(),
            "tool_choice 'none' should be omitted for Anthropic"
        );
    }
}
